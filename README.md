...
```
index.html        → la app completa (archivo autocontenido, generado)
src/
  engine.js       → motor matemático (fracciones exactas, parsers, solvers con pasos)
  app.js          → lógica de la interfaz
  styles.css      → estilos y temas
  mid.html        → marcado de la app
  top.html / bottom.html → fragmentos de ensamblado
  build.sh        → ensambla todo en un único index.html
  server.js       → servidor opcional (estático + puente de IA + llave permanente)
  test-engine.cjs → pruebas del motor matemático
```

Para reconstruir `index.html` tras editar los fuentes:
```bash
bash src/build.sh
```
...
## ✅ Pruebas
```bash
node src/test-engine.cjs   # 39+ pruebas del motor (fracciones, ecuaciones, derivadas…)
```
...
- La carpeta `data/` (llave del servidor) está **excluida** de este repositorio mediante `.gitignore`.
